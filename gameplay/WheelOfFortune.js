...
function OnBetsPlaced(data) {
  ...
  socket.on(events.OnBetsPlaced, async (data) => {
    ...
    singleNoBet = data.bet_amount_blue;
    ...
    let point = await service.getUserPoint(
      data.playerId,
      data.bet_amount_blue + data.bet_amount_red + data.bet_amount_yellow
    );
    console.log("Point", point);
    //if (data.betValue == 0) {
    if (
      data.bet_amount_blue == 0 &&
...
async function WinNosCalculator() {
  ...
  if (singleNoBet != 0 && Win_singleNo == 0) {
    winPoint = winPoint + singleNoBet * 2; //kitn lgn hai******** blue ke liye
  }
