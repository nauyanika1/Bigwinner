...
async function WinNosCalculator() {
  ...
  previousWin_single.push(Win_singleNo + Win_TwoNo);
  ...
  if (
    singleNoBet != 0 &&
    [2, 3, 4, 5, 6].indexOf(Win_singleNo + Win_TwoNo) != -1
  ) {
    winPoint = winPoint + singleNoBet * 2.2; //kitn lgn hai******** 2 to 6 ke liye
  }

  if (doubleNoBet != 1 && Win_singleNo + Win_TwoNo == 7) {
  ...
  console.log("Dice sum", Win_singleNo + Win_TwoNo);
  ...
  return {
    win_point: winPoint,
    singleNo: Win_singleNo + Win_TwoNo,
